...
func (a *azureApp) GetEnvVars() (map[string]string, error) {
	envVars := map[string]string{
		...
		"REQUESTS_CA_BUNDLE": a.profile.AppLocalCAPath(a.routeToApp.Name),
	}
...
func pickAzureApp(cf *CLIConf) (*azureApp, error) {
	...
	routeToApp, err := pickCloudApp(tc, profile, cf, types.CloudAzure, matchAzureApp)
	if err != nil {
		return nil, trace.Wrap(err)
	}

	return newAzureApp(tc, profile, cf, routeToApp)
}
