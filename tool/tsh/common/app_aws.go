...
func (a *awsApp) GetEnvVars() (map[string]string, error) {
	...
	envVars := map[string]string{
		...
		"AWS_CA_BUNDLE":         a.profile.AppLocalCAPath(a.GetAppName()),
	}
...
func pickAWSApp(cf *CLIConf) (*awsApp, error) {
	...
	routeToApp, err := pickCloudApp(tc, profile, cf, types.CloudAWS, matchAWSApp)
	if err != nil {
		return nil, trace.Wrap(err)
	}

	return newAWSApp(tc, profile, cf, routeToApp)
}
