...
|Windows desktops|Users authenticate to the Teleport Web UI. The Web UI initiates a WebSocket session with the Teleport Proxy Service, which forwards traffic through another WebSocket it creates with the Teleport Desktop Service. The Desktop Service forwards RDP traffic to the desired RDP server.|
...
client CLI, the Teleport Web UI, and Teleport Connect, in addition to native
client tools like `kubectl` and `psql`. Client tools route traffic through
agents, usually routed through the Teleport Proxy Service.

### Communication between agents and infrastructure resources

Teleport agents route user traffic to resources in your infrastructure. The
...
For self-hosted resources, administrators must distribute mutual TLS credentials
to infrastructure resources, which typically means adding the Teleport CA to the
resource's trust store and giving the resource a Teleport-signed certificate.
The former allows the resource to verify the agent's identity, and the latter
allows the agent to verify the resource's identity.
